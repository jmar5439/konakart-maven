...
public class Bluepay extends BasePaymentModule implements PaymentInterface
{
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        String mode = (sd.isTestMode() ? URLEncoder.encode("TEST", "UTF-8") : URLEncoder.encode(
                "LIVE", "UTF-8"));
        parmList
                .add(new NameValue("ACCOUNT_ID", URLEncoder.encode(sd.getBluepayAcctId(), "UTF-8")));
        ...
        parmList.add(new NameValue("CUSTOM_ID", URLEncoder.encode(Integer.toString(order.getId()),
                "UTF-8")));
        parmList.add(new NameValue("CUSTOM_ID2", URLEncoder.encode(Integer.toString(order
                .getCustomerId()), "UTF-8")));

        // Set the billing address
        parmList.add(new NameValue("COMPANY_NAME", URLEncoder.encode(order.getCustomerCompany(),
                "UTF-8")));
        ...
        parmList.add(new NameValue("ZIP", URLEncoder.encode(order.getBillingPostcode(), "UTF-8")));
        parmList.add(new NameValue("PHONE", URLEncoder
                .encode(order.getCustomerTelephone(), "UTF-8")));
        parmList.add(new NameValue("EMAIL", URLEncoder.encode(order.getCustomerEmail(), "UTF-8")));
        parmList
                .add(new NameValue("COUNTRY", URLEncoder.encode(order.getBillingCountry(), "UTF-8")));
    ...
    protected class StaticData
    {
        ...
        /**
         * @param bluepaySecretKey the bluepaySecretKey to set
         */
