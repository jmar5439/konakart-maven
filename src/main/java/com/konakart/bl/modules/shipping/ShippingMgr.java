...
public class ShippingMgr extends BaseMgr implements ShippingMgrIf
{
    ...
    public void refreshConfigs() throws Exception
    {
        synchronized (mutex)
        {
            ...
            sd.setShippingMaxWeight(mgr.getConfigurationValueAsBigDecimal(false, SHIPPING_MAX_WEIGHT,
                    new BigDecimal(1000000)));
            sd.setShippingBoxWeight(mgr.getConfigurationValueAsBigDecimal(false, SHIPPING_BOX_WEIGHT,
                    new BigDecimal(0)));
            sd.setShippingBoxPadding(mgr.getConfigurationValueAsBigDecimal(false, SHIPPING_BOX_PADDING,
                    new BigDecimal(0)));
            ...
            sd.setFreeShippingOver(mgr.getConfigurationValueAsBigDecimal(false, FREE_SHIPPING_OVER,
                    new BigDecimal(0)));
            ...
            int storeCountryInt = mgr.getConfigurationValueAsInt(false, STORE_COUNTRY);
            ...
            int languageId) throws Exception
    {
        ...
        {
            zone = taxMgr.getZonePerCountryAndCode(deliveryCountry.getId(), order
                    .getDeliveryState());
        }
        if (zone != null)
        {
        ...
        int numProducts = 0, numDigitalDownloads = 0, numBookableProds = 0, numGiftCertificates = 0, numFreeShipping = 0;
        ...
        for (int i = 0; i < order.getOrderProducts().length; i++)
        {
            ...
            // Don't consider digital downloads, gift certificates or bookable products
