...
public class ProdTileTag extends BaseTag
{
     ...
     * @param _prod
     */
    public void init(KKAppEng _eng, ProductIf _prod)
    {
        this.eng = _eng;
        this.prod = _prod;
    }
    ...
    public int doStartTag() throws JspException
    {
        try
        {
            if (eng == null)
            {
                setEng((com.konakart.al.KKAppEng) pageContext.getSession().getAttribute(
                        "konakartKey"));
            }
    ...
    public void renderTag(StringBuffer sb) throws JspException, KKAppException
    {
        ...
        } else
        {
            ...
            if (eng.isWishListEnabled())
            {
                ...
                sb.append(getImg("plus-button", "../images/plus-button.png",
                        getMsg("common.add.to.wishlist")));
    ...
    private void getImageLink(StringBuffer sb, String size)
    {
        sb.append(getStartA(null, "SelectProd.action?prodId=" + prod.getId()));
        sb.append(getImg(null, eng.getProdImageBase(prod) + "_1_" + size + ".jpg", prod.getName(), /* addBase */
                false));
        sb.append(END_A);
    }
    ...
    {
        sb.append(getStartA("item-title", "SelectProd.action?prodId=" + prod.getId()));
