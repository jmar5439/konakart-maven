...
public class PagingTag extends BaseTag
{
    ...
    public int doStartTag() throws JspException
    {
        try
        {
            ...
            if (showBack == 1)
            {
                sb.append("<a class=\"pagination-element previous-items\" href=\"" + action
                        + ".action?navDir=" + ProductMgr.navBack + ts
                        + " class=\"pageResults\"></a>");
            } else
            {
            ...
            for (java.util.Iterator<Integer> iterator = pageList.iterator(); iterator.hasNext();)
            {
                ...
                {
                    sb.append("<a href=\"" + action + ".action?navDir=" + pageNum + ts
                            + "  class=\"pagination-element\">" + pageNum + "</a>");
                }
            }
            if (showNext == 1)
            {
                sb.append("<a class=\"pagination-element next-items\" href=\"" + action
                        + ".action?navDir=" + ProductMgr.navNext + ts
                        + " class=\"pageResults\"></a>");
            } else
            {
