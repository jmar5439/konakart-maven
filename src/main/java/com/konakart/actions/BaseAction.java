...
public class BaseAction extends ActionSupport implements ServletRequestAware, SessionAware
{
    ...
    protected String handleException(HttpServletRequest request, Exception e)
    {
        ...
        log.error("A customer has received the following exception message:"
                + customerExceptionMessage.replace("<br>", " : ") + "<br>The full exception is:", e);

        if (appEng != null)
        {
            addActionError(appEng.getMsg("exception.detailed.message", customerExceptionMessage));
        }
