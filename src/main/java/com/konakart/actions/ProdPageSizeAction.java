 ...
 */
public class ProdPageSizeAction extends BaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
             ...
             */
            kkAppEng.getProductMgr().setMaxDisplaySearchResults(numProds);
            setKKCookie(TAG_PROD_PAGE_SIZE, Integer.toString(numProds), request,
                    response, kkAppEng);
            kkAppEng.getCustomerTagMgr().insertCustomerTag(TAG_PROD_PAGE_SIZE,
                    Integer.toString(numProds));
            
            DataDescriptorIf dd =  kkAppEng.getProductMgr().getDataDesc();
            if (dd != null)
            {
                dd.setLimit(numProds + 1);
                dd.setOffset(0);                
                kkAppEng.getProductMgr().orderCurrentProds(dd.getOrderBy(), t);
            }
    ...
    /**
     * @param numProds the numProds to set
     */
