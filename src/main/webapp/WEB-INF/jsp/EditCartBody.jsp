...
$(function() {	
		...
		$('.update-button').click(function() {
			  ...
			  if (basketId == null) {
				  ...
				  qtyMap[inputId] = qty;
				  elem.hide();
				  window.location.href = 'EditCartSubmit.action?action=q&id='+basketId+'&qty='+qty;
			  }
		});
	});
</script>
		    						...
		    						<tr>
		    							<td>
		    								<%if ((item.getQuantity() > item.getQuantityInStock()) && prodMgr.isStockCheck()) { %>
													...
													<% outOfStock=true; %>
											<% } %>
		    								<img class="product-image" src="<%=kkEng.getProdImageBase(item.getProduct())+"_1_tiny.jpg"%>" border="0" alt="<%=item.getProduct().getName()%>" title="<%=item.getProduct().getName()%>"/>
		    							</td>
		    							<td>		    								
										...
										<% } %>		    							
		    							<td class="center"><a href="EditCartSubmit.action?action=r&id=<%=item.getId()%>" class="has-tooltip" title="<kk:msg  key="common.remove.item"/>"><img src="<%=kkEng.getImageBase()%>/x-button.png" border="0"></a></td>
		    					    </tr>
								<%}%>
