...
$(function() {	
	...
	$('.update-button').click(function() {
		  ...
		  if (itemId == null) {
			  ...
			  qtyMap[inputId] = qty;
			  elem.hide();
			  window.location.href = 'EditWishListSubmit.action?action=q&wid='+wlId+'&id='+itemId+'&qty='+qty;
		  }
	});
});
								...
								<%for (java.util.Iterator<com.konakart.al.WishListUIItem> iterator = itemList.iterator(); iterator.hasNext();){%>
					               ...
					               <input type="hidden" name="itemList[<%=k%>].wishListItemId" value="<%=item.getWishListItemId()%>"/>
		    						<tr>
		    							...
		    							<td class="center">
		    								<a href="EditWishListSubmit.action?action=r&wid=<%=id%>&id=<%=item.getWishListItemId()%>" class="has-tooltip" title="<kk:msg  key="common.remove.item"/>"><img src="<%=kkEng.getImageBase()%>/x-button.png" border="0"></a>
		    							</td>
		    					    </tr>
		    					    <% k++; %>	
								<%}%>
