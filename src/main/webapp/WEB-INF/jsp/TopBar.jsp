											...
											<% for (int j = 0; j < wishList.getWishListItems().length; j++){ %>
												...
												<%if (item.getProduct() != null) { %>		
													<div class="shopping-cart-item">
											  			<a href="SelectProd.action?prodId=<%=item.getProduct().getId()%>"><img src="<%=kkEng.getProdImageBase(item.getProduct())+"_1_tiny.jpg"%>" border="0" alt="<%=item.getProduct().getName()%>" title=" <%=item.getProduct().getName()%> "></a>
